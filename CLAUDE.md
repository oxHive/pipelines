...
`publish-crates` and `publish-homebrew` are opt-in (`false` by default) — callers must set the matching boolean input to run either job. `publish-homebrew` additionally requires `homebrew-install` to be non-empty. See `docs/homebrew-github-app.md` for setting up `HOMEBREW_TAP_TOKEN` via a GitHub App.
